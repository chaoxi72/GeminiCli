...
export class CustomLLMContentGenerator implements ContentGenerator {
  ...
  constructor() {
    // 如果设置了绕过认证且使用自定义LLM，使用默认值以避免验证失败
    if (process.env['BYPASS_AUTH'] === 'true' && process.env['USE_CUSTOM_LLM'] === 'true') {
      this.apiKey = this.apiKey || 'bypass-auth-key';
      this.baseURL = this.baseURL || 'http://localhost:8080/v1';
      this.modelName = this.modelName || 'custom-model';
